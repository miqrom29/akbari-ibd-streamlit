...
if input_mode == "ancIBD block TSV (no header)":
    ...
    anc_file = st.sidebar.file_uploader(
        "Block TSV: unindented focal ID + tab-indented match\\tcM lines",
        type=["tsv","txt","csv"], key="ancibd"
    )
    if anc_file is None:
        st.info("📂 Upload an ancIBD block TSV file to begin.\n\n"
                "**Format (no header):**\n```\nFocalSample\n\tMatch1\t2171.90\n\tMatch2\t31.11\n```\n"
                "Multiple focal sample blocks in the same file are supported.")
        st.stop()
    raw = anc_file.getvalue()
    if not is_ancibd_block_format(raw):
        st.error("⚠️ File does not look like an ancIBD block TSV. "
                 "Make sure the focal sample ID is on its own unindented line "
                 "and matches are on tab-prefixed lines below it.")
        st.stop()
